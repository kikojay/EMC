...
from modules.mixers.qtran_transformation import QTRAN_transformation
import torch as th
...
class QLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.transformation = QTRAN_transformation(args)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, show_demo=False, save_data=None, show_v=False):
        ...
        max_actions_qvals = self.transformation(max_actions_qvals, batch["state"])
        chosen_action_qvals = self.transformation(chosen_action_qvals, batch["state"][:, :-1])
        ...
        if show_v:
            ...
            self.logger.log_stat("test_q_taken_mean", ((joint_qs * mask.reshape(-1, 1)).sum().item()/mask_elems), t_env)
            return
    ...
    def cuda(self):
        ...
        to_cuda(self.transformation, self.args.device)
    ...
    def save_models(self, path):
        ...
        th.save(self.transformation.state_dict(), "{}/transformation.th".format(path))
        if self.mixer is not None:
    ...
    def load_models(self, path):
        ...
        self.transformation.load_state_dict(th.load("{}/transformation.th".format(path), map_location=lambda storage, loc: storage))
